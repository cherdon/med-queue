...
function redrawWindow(){
	...
	document.getElementById("time").innerHTML = "Time Elapsed: 0min"; //reset timer on top left
...
function updateSurface(){
	...
	allstatistics.selectAll("text").text(function(d) {
		...
		return d.name + avgLengthOfStay.toFixed(1) + "mins" + " (" + d.count +" treated)" + " [" + d.rejected + " rejected]"; });
	...
	newcaregivers.append("svg:image")
		...
		.attr("width", Math.min(config.cellWidth,config.cellHeight)+"px")
		.attr("height", Math.min(config.cellWidth,config.cellHeight)+"px")
		.attr("xlink:href", config.receptionistImage);	
	// Add caregiver label
	...
	// Drawing all aesthetic staff
	var allstaff = surface.selectAll(".staff").data(visual.aesthetic_staff);
	...
	newstaff.append("svg:image")
		...
		.attr("width", Math.min(config.cellWidth,config.cellHeight)+"px")
		.attr("height", Math.min(config.cellWidth,config.cellHeight)+"px")
		.attr("xlink:href", function(d){if(d.type == EMERGENCY_DOCTOR)return config.doctorImage; else return config.receptionistImage});
	// Add doctor labels
...
function probabilityDelay(model, variables){
	if (model == POISSON){
		return poissonDistribution(variables.rate)
	}
	else if (model == UNIFORM){
		return uniformDistribution(variables.min, variables.max)
	}
	else if (model = NORMAL){
		return Math.floor(normalDistribution(variables.mean, variables.stdev))
	}
	else {
...
function timeParser(totalTime){
	var days = Math.floor(Math.floor(totalTime / 60)/24)
	var hours = Math.floor((totalTime - (days*24*60))/60)
	var minutes = totalTime % 60

	var timeString = ""
	if (days > 0){
		timeString = timeString + String(days) + "Days, "
	}
	if (hours > 0){
...
function simStep(){
	if (config.isRunning){ 
		...
		currentTime++;
		document.getElementById("time").innerHTML = "Time Elapsed: " + timeParser(currentTime);
