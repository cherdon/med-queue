...
var patients = [];
var doctor = visual.caregivers[0]; // the doctor is the first element of the caregivers list.

// We can section our screen into different areas. In this model, the waiting area and the staging area are separate.
var waitingRoom = visual.areas[0]; // the waiting room is the first element of the areas array

var currentTime = 0;
var statistics = [
{"name":"Average time in clinic, Type A: ","location":{"row":config.doctorRow+3,"col":config.doctorCol-4},"cumulativeValue":0,"count":0},
{"name":"Average time in clinic, Type B: ","location":{"row":config.doctorRow+4,"col":config.doctorCol-4},"cumulativeValue":0,"count":0}
];
...
function updateSurface(){
	...
	newpatients.append("svg:image")
	 ...
	 .attr("xlink:href",function(d){if (d.type=="A") return config.patientA; else return config.patientB;});
	...
	newcaregivers.append("svg:image")
	 ...
	 .attr("width", Math.min(config.cellWidth,config.cellHeight)+"px")
	 .attr("height", Math.min(config.cellWidth,config.cellHeight)+"px")
	 .attr("xlink:href",function(d){if (d.type==DOCTOR) return config.doctor1; else return config.receptionist;});
	...
	.attr("x", function(d){return (d.startCol-1)*config.cellWidth;})
	.attr("y",  function(d){return (d.startRow-1)*config.cellHeight;})
...
function addDynamicAgents(){
	...
	if (Math.random()< probArrival){
		...
		"target":{"row":config.receptionistRow,"col":config.receptionistCol},"state":UNTREATED,"timeAdmitted":0};
	...
	switch(state){
			...
			switch (type){
				case "A":
					if (patient.id == nextTreatedPatientID_A){
						patient.target.row = config.doctorRow-1;
						patient.target.col = config.doctorCol-1;
						patient.state = STAGING;
					}
					if (patient.id == nextTreatedPatientID_A+1){
						patient.target.row = config.doctorRow-1;
						patient.target.col = config.doctorCol-2;
					}
				break;
				case "B":
		...
		case STAGING:
			...
			if (hasArrived){
				...
				if (doctor.state == IDLE){
					...
					patient.target.row = config.doctorRow;
					patient.target.col = config.doctorCol;
		...
		case INTREATMENT:
			...
			if (Math.random()< probDeparture){
				...
				doctor.state = IDLE;
				patient.target.row = config.receptionistRow;
				patient.target.col = config.receptionistCol;
			}
		break;
		case TREATED:
			if (hasArrived){
				...
				patient.target.col = config.maxCols;
